...
def create_test_data(conn):
    ...
    try:
        ...
        conn.execute(text("""
            INSERT INTO users (username, email, hashed_password, is_captain, team_id)
            VALUES ('testuser', 'test@example.com', :password, TRUE, :team_id)
        """), {"password": hashed_password, "team_id": team_id})
        
        # Создаем тестовые задания
        challenges = [
            ("Web Security 101", "Базовое задание по веб-безопасности", "WEB", "easy", 100, "CTF{web_security_basic}"),
            ("Crypto Challenge", "Задание по криптографии", "Crypto", "medium", 200, "CTF{crypto_is_fun}"),
            ("Forensics Investigation", "Расследование цифровых улик", "Forensics", "hard", 300, "CTF{forensics_master}")
        ]
        
        for title, description, category, difficulty, points, flag in challenges:
            conn.execute(text("""
